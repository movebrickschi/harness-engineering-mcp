...
参见 `docs/M3_CURSOR_INTEGRATION.md`。要点：
...
Claude Code 通过同样的 MCP JSON-RPC 协议挂载 MCP server。配置示例：
...
> Windows 上若 PATH 中找不到 `harness-mcp`，把 `command` 改为 `node` 并 `args` 指向 `dist/mcp-server.js` 绝对路径。
...
npx -y harness-engineering-mcp@0.1.0 --version
npx -y harness-engineering-mcp@0.1.0 init --mode=solo --stack=other --type=library --name=verify
npx -y harness-engineering-mcp@0.1.0 check --json
